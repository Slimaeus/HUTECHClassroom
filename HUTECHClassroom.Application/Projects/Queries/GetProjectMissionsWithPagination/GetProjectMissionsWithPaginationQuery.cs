...
public sealed record GetProjectMissionsWithPaginationQuery(Guid Id, ProjectPaginationParams Params) : GetWithPaginationQuery<MissionDTO, ProjectPaginationParams>(Params);
public sealed class GetProjectMissionsWithPaginationQueryHandler : GetWithPaginationQueryHandler<Mission, GetProjectMissionsWithPaginationQuery, MissionDTO, ProjectPaginationParams>
{
    ...
    protected override Expression<Func<Mission, bool>> FilterPredicate(GetProjectMissionsWithPaginationQuery query)
        => x => x.ProjectId == query.Id && (query.Params.UserId == null || query.Params.UserId == Guid.Empty || x.MissionUsers.Any(mu => query.Params.UserId == mu.UserId));
    ...
    protected override Expression<Func<Mission, bool>> SearchStringPredicate(string searchString)
    {
        ...
        return x => x.Title.ToLower().Contains(toLowerSearchString) || (x.Description != null && x.Description.ToLower().Contains(toLowerSearchString));
    }
}
